...
import { GENERAL_CATEGORIES, CONTACT_UI } from "@/lib/contactUi";
...
export default function GeneralInquiryTab() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (form.message.trim().length < 5)
      return setError("請填寫諮詢內容（至少 5 字）");
    if (!agreed) return setError("請同意個人資料處理方式");

    const catLabel =
      GENERAL_CATEGORIES.find((c) => c.value === form.category)?.label ||
      form.category;
    ...
    return (
      <div className="p-8 sm:p-12 text-center bg-white">
        ...
        <button
          ...
          className="text-sm font-bold text-[#005caf] hover:underline"
        >
          再送一則諮詢
        </button>
      </div>
    ...
    <form onSubmit={handleSubmit}>
      ...
      <div className="px-4 sm:px-6 py-5 bg-white border-t border-slate-200 space-y-4">
        ...
        <button
          type="submit"
          disabled={submitting}
          className="w-full sm:w-auto min-w-[240px] flex items-center justify-center gap-2 px-8 py-3.5 text-white text-sm font-bold rounded-sm disabled:opacity-50 transition hover:bg-[#1a3a8a]"
