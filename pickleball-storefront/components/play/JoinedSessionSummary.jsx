...
export default function JoinedSessionSummary({ session }) {
  ...
  const dateText = formatSessionDate(session.starts_at);
  const timeText = formatSessionRange(session.starts_at, session.ends_at);
    ...
    <StatusPanel>
      ...
      <div className="mt-5 space-y-4 border-t border-white/10 pt-5">
        ...
        {(session.max_players || session.skill_level) && (
          <InfoRow icon={Users} label="場次">
            <span className="inline-flex flex-wrap items-center gap-2">
              {session.max_players && (
                <span>最多 {session.max_players} 人</span>
              )}
              {session.skill_level && (
